 ...
 */
// @formatter:on
public final class JsonAppendableWriter 
extends JsonWriterBase<JsonAppendableWriter> 
implements JsonSink<JsonAppendableWriter> {
	...
	/** Closes this JSON writer and flushes the underlying {@link Appendable} 
	 * if it is also {@link Flushable}.
	 * 
	 * @throws JsonWriterException if the underlying {@link Flushable} 
	 * {@link Appendable} failed to flush. */
	public void done() throws JsonWriterException {
		super.doneInternal();
		try {
			if (appendable instanceof Flushable f) f.flush();
