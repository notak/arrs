...
import static utils.arrays.Objs.mapLong;
...
public class Longs {
	...
	/** Sort with comparator. Kind of ugly because it has to box/unbox */
	public static long[] sorted(long[] in, Comparator<Long> sort) {
		if (in.length<2) return in;
		var out = map(in, i->i, Long[]::new);
		sort(out, sort);
		return mapLong(out, i->i);
	}

	/** Sort with comparator. Kind of ugly because it has to box/unbox */
	public static <T> long[] sorted(long[] in, ToIntFn sort) {
		return sorted(in, (a, b)->sort.apply(a)-sort.apply(b));
	}
}
