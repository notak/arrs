...
import static utils.arrays.Objs.nthArray;
import static utils.arrays.Objs.nthBoolean;
...
import static utils.json.JsonObject.nthObject;
...
public class JsonParserTest {
	...
	public void testEmptyArray() throws JsonParserException {
		assertEquals(Object[].class, JsonParser.array().from("[]").getClass());
		assertEquals("[]", Arrays.toString(JsonParser.array().from("[]")));
	}

	@Test
	public void testArrayOneElement() throws JsonParserException {
		assertEquals(Object[].class, JsonParser.array().from("[1]").getClass());
		assertEquals("[1]", Arrays.toString(JsonParser.array().from("[1]")));
	}

	@Test
	public void testArrayTwoElements() throws JsonParserException {
		assertEquals(Object[].class, JsonParser.array().from("[1,1]").getClass());
		assertEquals("[1, 1]", Arrays.toString(JsonParser.array().from("[1,1]")));
	}
	...
	public void testArrayWithEverything() throws JsonParserException {
		var a = JsonParser.array().from("[1, -1.0e6, \"abc\", [1,2,3], {\"abc\":123}, true, false]");
		assertEquals("[1, -1000000.0, abc, [1, 2, 3], {abc=123}, true, false]", Arrays.deepToString(a));
		assertEquals(1.0, nthDouble(a, 0, 0), 0.001f);
		assertEquals(1, nthInt(a, 0, 0));
		assertEquals(-1000000, nthInt(a, 1, 0));
		assertEquals(-1000000, nthDouble(a, 1, 0), 0.001f);
		assertEquals("abc", nthString(a, 2, ""));
		assertEquals(1, nthInt(nthArray(a, 3, null), 0, 0));
		assertEquals(123, nthObject(a, 4, null).getInt("abc"));
		assertTrue(nthBoolean(a, 5, false));
		assertFalse(nthBoolean(a, 6, true));
	}
	...
	public void testObjectWithEverything() throws JsonParserException {
		...
		assertEquals(Arrays.asList(true, false), Arrays.asList((Object[])o.get("ghi")));
		...
		assertTrue(nthBoolean(o.getArray("ghi"), 0, false));
		assertEquals(null, o.get("jkl"));
	...
	public void testFailTrailingCommaUTF8() {
		...
		try {
			...
			testException(e, e1.linePos, e1.charPos);
		}
	}
